...
public class Node {
	...
	// Constructor
	// -----------------------------------------------------------------------------------------
	public Node(int homeId, byte nodeId, Controller primaryController, QueueManager queue, XMLManager xml, Log log) {
		this.homeId = homeId;
		this.nodeId = nodeId;
	...
	private void setDeviceClasses(byte basic, byte generic, byte specific) {
		...
		genericId = generic;
		specificId = specific;
		deviceType = xml.getBasicDeviceClasses().containsKey(basic) ? xml.getBasicDeviceClasses().get(basic) : "";
		...
		if (xml.getGenericDeviceClasses().containsKey(generic)) {
			...
			ccManager.addCommandClasses(genericDeviceClass.getMandatoryCommandClasses());
			basicMapping = genericDeviceClass.getBasicMapping();

			if (genericDeviceClass.getSpecificDeviceClasses().containsKey(specific)) {
				deviceClass = genericDeviceClass.getDeviceClass(specific);
				deviceType = deviceClass.getLabel();

				ccManager.addCommandClasses(deviceClass.getMandatoryCommandClasses());
			...
			if (!listeningDevice) {
				WakeUp wu = (WakeUp) ccManager.getCommandClass(WakeUp.COMMAND_CLASS_ID);
				if (wu != null) {
	...
	public boolean isController() {
		return (basicId == 0x01 || basicId == 0x02) && (genericId == 0x01 || genericId == 0x02);
	}
	...
	private void setStaticRequest() {
		...
		if (request != 0) {
			for (Entry<Byte, CommandClass> entry : ccManager.getCommandClassMap().entrySet()) {
	...
	public void advanceQueries() {
		...
		while (nodeAlive && !queryPending) {
			log.add(String.format("QueryStage: [Node %d] %s", SafeCast.toInt(nodeId), queryStage.name()));
			switch (queryStage) {
			case None:
			...
			case ProtocolInfo:
				if (!protocolInfoReceived) {
					msg = new Msg(nodeId, Defs.REQUEST, Defs.FUNC_ID_ZW_GET_NODE_PROTOCOL_INFO, false);
					msg.append(nodeId);
				...
				break;
			case Probe1:
				NoOperation noop1 = (NoOperation) ccManager.getCommandClass(NoOperation.COMMAND_CLASS_ID);
	...
	public void queryStageRetry(QueryStage stage, byte maxAttempts) {
		...
		if (maxAttempts != 0 && (++queryRetries >= maxAttempts)) {
			queryRetries = 0;
			if (queryStage != QueryStage.Probe1 && queryStage != QueryStage.Probe2) {
	...
	public boolean requestDynamicValues() {
		...
		for (Entry<Byte, CommandClass> entry : ccManager.getCommandClassMap().entrySet()) {
			...
			if (!cc.isAfterMark()) {
				res |= cc.requestStateForAllInstances(CommandClass.REQUEST_FLAG_DYNAMIC, QueuePriority.Send);
			}
		}
		return res;
	}
	...
	public boolean requestSessionValues() {
		...
		for (Entry<Byte, CommandClass> entry : ccManager.getCommandClassMap().entrySet()) {
			CommandClass cc = entry.getValue();
			if (!cc.isAfterMark()) {
				res |= cc.requestStateForAllInstances(CommandClass.REQUEST_FLAG_SESSION, QueuePriority.Query);
			}
		}
		return res;
	}
	...
	public boolean requestStaticValues() {
		...
		for (Entry<Byte, CommandClass> entry : ccManager.getCommandClassMap().entrySet()) {
			CommandClass cc = entry.getValue();
			if (!cc.isAfterMark()) {
				res |= cc.requestStateForAllInstances(CommandClass.REQUEST_FLAG_STATIC, QueuePriority.Query);
			}
		}
		return res;
	}
	...
	public boolean requestAllVersion() {
		boolean res = false;
		Version vcc = (Version) ccManager.getCommandClass(Version.COMMAND_CLASS_ID);
		if (vcc != null) {
			for (Entry<Byte, CommandClass> entry : ccManager.getCommandClassMap().entrySet()) {
				CommandClass cc = entry.getValue();
	...
	public boolean requestAllConfiguration(int requestFlags) {
		boolean res = false;
		Configuration cc = (Configuration) ccManager.getCommandClass(Configuration.COMMAND_CLASS_ID);
		if (cc != null) {
			for (Entry<Integer, Value> entry : values.entrySet()) {
				Value val = entry.getValue();
				if (val.getId().getCommandClassId() == Configuration.COMMAND_CLASS_ID && !val.isWriteOnly()) {
					res |= cc.requestValue(requestFlags, val.getId().getIndex(), (byte) 1, QueuePriority.Send);
				}
			}
		}
		return res;
	...
	public String displayValues(){
		...
		while(it.hasNext()){
			...
			if(val.getValue().getLabel() == "Power"){
				Value vd = val.getValue();
				result += "POWER###########: " + ((ValueDecimal)vd).getValue() + "\n";
			}
			result += val.toString() + "# " + val.getValue().getLabel() + "\n";
		}
		return result;
	}
	
	
	public NodeToSend getNodeToSend(){
		Boolean onOffController;
		if( this.getCommandClassManager().getCommandClass(SwitchBinary.COMMAND_CLASS_ID) != null )
    		onOffController = true;
    	else onOffController = false;
    	
		NodeToSend node = new NodeToSend(getNodeId(), getNodeName(), onOffController, isBeamingDevice(), isController());
    	
    	ArrayList<Information> informations = new ArrayList<Information>();
    	
    	Iterator it = this.getValueManager().entrySet().iterator();
		while(it.hasNext()){
			Entry<Integer, Value> val = (Entry<Integer, Value>)it.next();
			...
			if(tmp.getId().getGenre() == ValueId.ValueGenre.USER){
				switch (tmp.getId().getType()) {
					case BOOL:
						info.setValue(Boolean.toString( ((ValueBool)tmp).getValue() ));
						info.setName(((ValueBool)tmp).getLabel());
						info.setUnit(((ValueBool)tmp).getUnits());
						break;
					case BYTE:
						info.setValue(Byte.toString( ((ValueByte)tmp).getValue() ));
						info.setName(((ValueByte)tmp).getLabel());
						info.setUnit(((ValueByte)tmp).getUnits());
						break;
					case DECIMAL:
						info.setValue(((ValueDecimal)tmp).getValue());
						info.setName(((ValueDecimal)tmp).getLabel());
						info.setUnit(((ValueDecimal)tmp).getUnits());
						break;
					case INT:
						info.setValue(Integer.toString( ((ValueInt)tmp).getValue()) );
						info.setName(((ValueInt)tmp).getLabel());
						info.setUnit(((ValueInt)tmp).getUnits());
						break;
					case LIST:
					...
					case SCHEDULE:
						...
						break;
					case SHORT:
						info.setValue(Short.toString( ((ValueShort)tmp).getValue()) );
						info.setName(((ValueShort)tmp).getLabel());
						info.setUnit(((ValueShort)tmp).getUnits());
						break;
					case STRING:
						info.setValue(((ValueString)tmp).getValue());
						info.setName(((ValueString)tmp).getLabel());
						info.setUnit(((ValueString)tmp).getUnits());
						break;
					case BUTTON:
