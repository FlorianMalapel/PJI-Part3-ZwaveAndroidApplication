...
public abstract class XMLManager {
	...
	private void readCommandClasses(com.azwave.androidzwave.zwave.nodes.Node node, Document doc)
			throws Exception {
		...
		for (int i = 0; i < xmlnodes.getLength(); i++) {
			...
			if (idstr != null && idstr.length() > 0) {
				...
				boolean remove = action != null && action.length() > 0 && action.compareToIgnoreCase("remove") == 0;

				CommandClass cc = node.getCommandClassManager().getCommandClass(id);
				if (remove) {
