...
public class CommandClassManager {
	...
	public CommandClass addCommandClass(byte commandClassId) {
		if (commandClassMap.containsKey(commandClassId) | !isSupported(commandClassId)) {
			return null;
		}
	...
	public static String getString(byte commandClassId) {
		...
		case Meter.COMMAND_CLASS_ID:
			return Meter.COMMAND_CLASS_NAME;
	...
	public void displayCommandClasses(){
		Log.d("DEBUG", "==========================");
		Iterator it = commandClassMap.entrySet().iterator();
		while(it.hasNext()){
			Entry cmd = (Entry)it.next();
			Log.d("DEBUG", "" + cmd.getValue());
		}
		Log.d("DEBUG", "==========================");
	}
