...
public class PowerLevel extends CommandClass {
	...
	public static final String[] powerLevelStatusNames = { "Failed", "Success", "In Progress" };
	...
	@Override
	public boolean requestValue(int requestFlags, byte index, byte instance, QueuePriority queue) {
		if (index == 0) {
			Msg msg = Msg.createZWaveApplicationCommandHandler(getNodeId(), COMMAND_CLASS_ID);
			msg.setInstance(this, instance);
			msg.appends(new byte[] { getNodeId(), 2, COMMAND_CLASS_ID, POWER_LVL_CMD_GET,
	...
	public boolean set(byte instance) {
		...
		ValueList vallist = (ValueList) getValue(instance, (byte) PowerlevelIndex.Powerlevel.ordinal());
		if (vallist != null) {
		...
		ValueByte valbyte = (ValueByte) getValue(instance, (byte) PowerlevelIndex.Timeout.ordinal());
		if (valbyte != null) {
